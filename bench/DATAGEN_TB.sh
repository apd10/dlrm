# this wil generate the terabyte dataset from unzipped day_ files. Also this takes apprx 2 days of time
...
$dlrm_pt_bin --arch-sparse-feature-size=64 --arch-mlp-bot="13-512-256-64" --arch-mlp-top="512-512-256-1" --max-ind-range=-1 --data-generation=dataset --data-set=terabyte --raw-data-file=./input/day --processed-data-file=./input/terabyte_processed.npz --loss-function=bce --round-targets=True --learning-rate=0.1 --mini-batch-size=2048 --print-freq=1024 --print-time --test-mini-batch-size=16384 --test-num-workers=16 $dlrm_extra_option --mlperf-logging --mlperf-bin-loader --mlperf-bin-shuffle --memory-map --dataset-multiprocessing --use-gpu  2>&1 | tee run_terabyte_pt.log
